...
/* if ((call BIS_fnc_admin) != 0) then {
    ...
    _btnControl ctrlAddEventHandler["ButtonClick", {[GVAR(ServerCommandPassword), '#init'] remoteExecCall ["serverCommand", 2];}];
}; */
...
if ((call BIS_fnc_admin) != 0) then {
    ...
    _btnControl ctrlAddEventHandler["ButtonClick", {
        ...
        [GVAR(ServerCommandPassword), '#init'] remoteExecCall ["serverCommand", 2];
        }];
};
