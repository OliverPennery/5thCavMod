...
private ["_newSides","_newList"];
if (GVAR(showSpectatorList)) then {
    _newSides = [str west, str east, str resistance, str civilian, "sideSpectator"];
    _newList = [
        ...
        [sideLogic, "sideSpectator", "Spectators", getArray (configFile >> "CfgMarkerColors" >> QEGVAR(common,ColourJedAlpha) >> "color") call BIS_fnc_colorConfigToRGBA, []]
    ];
} else {
    _newSides = [str west, str east, str resistance, str civilian];
    _newList = [
        [west, str west, [west] call BIS_fnc_sideName, [west] call BIS_fnc_sideColor, []],
        [east, str east, [east] call BIS_fnc_sideName, [east] call BIS_fnc_sideColor, []],
        [resistance, str resistance, [resistance] call BIS_fnc_sideName, [resistance] call BIS_fnc_sideColor, []],
...
private _entities = [true] call SFUNC(ace,spectator,getTargetEntities);
{
    ...
    if !(str _group in _newGroups) then {
        ...
        _entitiesGroup = _entitiesGroup select {alive _x};
        ...
        if !(_entitiesGroup isEqualTo []) then {
            ...
            private _groupTexture = [_group] call SFUNC(ace,spectator,getGroupIcon);
            private _grpConfig = [_group] call EFUNC(common,getGroup);
            private "_groupColor";
            if (!isNil "_grpConfig") then {
                _groupColor = getArray (configFile >> "CfgMarkerColors" >> (_grpConfig select 2) >> "color") call BIS_fnc_colorConfigToRGBA;
            } else {
                _groupColor = getArray (configFile >> "CfgMarkerColors" >> "colorBrown" >> "color") call BIS_fnc_colorConfigToRGBA;
            };
            private _groupInfo = [_group, str _group, _groupTexture, groupID _group, _groupColor];
...
if (GVAR(showSpectatorList)) then {
    ...
    {
        _newUnits pushBack ([_x] call BIS_fnc_objectVar);
        _spectators pushBack [
            _x,
            false,
            false,
            ([_x] call SFUNC(ace,common,getName)) select [0, NAME_MAX_CHARACTERS]
        ];
    } forEach ([] call SFUNC(ace,spectator,players));
    if !(_spectators isEqualTo []) then {
        _newGroups pushBack "fake_spectator_group";
        ((_newList select (_newSides find "sideSpectator")) select 4) pushBack [["","fake_spectator_group",ICON_INF,"Spectators", getArray (configFile >> "CfgMarkerColors" >> QEGVAR(common,ColourJedAlpha) >> "color") call BIS_fnc_colorConfigToRGBA],_spectators]
    };
};

// Whether an update to the list is required (really only if something changed)
if !(SGVAR(ace,spectator,curList) isEqualTo _newList) then {
    ...
    // Update/add the values
    {
...

        {
            ...
            _groupInfo params ["_group", "_groupStr", "_groupTexture", "_groupId", "_groupColor"];
            ...
            if (_lookup < 0) then {
                ...
                if (GVAR(enableUnitIcons)) then {
                    _ctrl tvSetPictureColor [[_sideIndex, _groupIndex], _groupColor];
                } else {
                ...
                if (GVAR(enableUnitIcons)) then {
                    _ctrl tvSetPictureColor [_path, _groupColor];
                } else {
...

            {
                ...
                private _tooltip = [[_unit] call SFUNC(ace,common,getName)];
                if ([_unit] call SFUNC(ace,common,isMedic)) then { _tooltip pushBack (localize "str_support_medic"); };
                if ([_unit] call SFUNC(ace,common,isEngineer)) then { _tooltip pushBack (localize "str_ace_spectator_TooltipEngineer"); };
                ...
                private _texture = [_isAlive, _isIncapacitated, _unit] call {
                    ...
                    [vehicle _unit] call SFUNC(ace,common,getVehicleIcon)
                };
    ...
    SGVAR(ace,spectator,curList) = _newList;
};

// Update focus if required
[] call SFUNC(ace,spectator,ui_updateListFocus);
